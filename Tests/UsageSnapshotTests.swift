...
enum UsageSnapshotTests {
    static func main() throws {
        ...
        check(mapped.preferredEntry?.window.label == "Week", "Primary can be weekly")
        ...
        for prefix in ["", #""rateLimitsByLimitId":null,"#] {
            ...
            check(legacy.windows[0].window.label == "5 hours", "Five-hour label")
        ...
        check(unknown.windows[0].window.label == "Unspecified period", "Never infer missing duration")
        ...
        check(RateLimitWindow(usedPercent: .nan, windowDurationMins: 90, resetsAt: nil).label == "90 minutes", "Report actual duration")
